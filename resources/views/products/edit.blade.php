...
@section('content')
    ...
    <div class="rounded-xl border border-slate-200 bg-white p-6">
        <h2 class="mb-6 text-lg font-semibold text-slate-800">Editar Produto</h2>
        ...
        <form action="{{ route('products.update', $product) }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3"
                          ...
                          placeholder="Descrição opcional do produto">{{ old('description', $product->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="number" id="price_in_cents" name="price_in_cents" value="{{ old('price_in_cents', $product->price_in_cents) }}"
            ...
            <div class="flex items-center gap-2">
                <input type="checkbox" id="active" name="active" value="1"
                       @if(old('active', $product->active)) checked @endif
