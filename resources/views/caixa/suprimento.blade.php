...
@section('content')
    ...
    <div class="rounded-xl border border-slate-200 bg-white p-6">
        <h2 class="mb-6 text-lg font-semibold text-slate-800">Suprimento</h2>

        <form action="{{ route('caixa.suprimento') }}" method="POST" class="space-y-4">
            @csrf
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3"
                          ...
                          placeholder="Ex: Reforço de troco">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center gap-3 pt-2">
                <button type="submit"
                        class="rounded-lg bg-blue-500 px-5 py-2 text-sm font-medium text-white transition hover:bg-blue-600">
